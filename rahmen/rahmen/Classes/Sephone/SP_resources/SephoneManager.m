...
+ (void)addProxyConfig:(NSString *)username password:(NSString *)password domain:(NSString *)domain {
    ...
    if (domain && [domain length] != 0) {
        ...
        //如果有冒号（有端口）
        if ([domain rangeOfString:portString].location != NSNotFound) {
            sephone_address_set_port(address, 6060);
            if ([domain rangeOfString:tcpString].location != NSNotFound) {
                sephone_address_set_transport(address, SephoneTransportTcp);
            }else{
                sephone_address_set_transport(address, SephoneTransportUdp);
            }
            //直接取端口前面的就行了
            NSRange   i = [domain rangeOfString:@":"];
            NSInteger a = i.location;
            NSString * str = [domain substringToIndex:a];
            sephone_address_set_domain(address, [str UTF8String]);
            
        }else{
          // sephone_address_set_port(address, 5060);
            if ([domain rangeOfString:tcpString].location != NSNotFound) {
                sephone_address_set_transport(address, SephoneTransportTcp);
                //没有端口，有tcp，直接取；前面的
                NSRange   i = [domain rangeOfString:@";"];
