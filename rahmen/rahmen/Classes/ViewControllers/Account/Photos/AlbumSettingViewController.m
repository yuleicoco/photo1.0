...
static NSString * cellId = @"AlbumphotoTableViewCellId2";
...
- (void)viewDidLoad {
    ...
    [self setNavTitle:@"Albun Setting"];
...
//    }];
    ...
    [[AFHttpClient sharedAFHttpClient]compliePhoto:[AccountManager sharedAccountManager].loginModel.userid token:@"" albumname:_nameTextfield.text dids:didstr aid:_aidStr complete:^(BaseModel *model) {
        if (model) {
            [self.navigationController popViewControllerAnimated:NO];
...
-(void)setupData{
    ...
    [[AFHttpClient sharedAFHttpClient]albumdetail:[AccountManager sharedAccountManager].loginModel.userid token:@"" aid:_aidStr complete:^(BaseModel *model) {
        if (model) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    _nameTextfield.text = model.albumname;
